...
/**
 * @file
 * gumby-links.func.php
 */

/**
 * Implements theme_gumby_links().
 */
function theme_gumby_links($variables) {
  ...
  if (count($links) > 0) {
    ...
    foreach ($links as $key => $link) {
      ...
      if (count($children) > 0) {
        ...
        $output .= theme('gumby_links', array('links' => $children, 'attributes' => $attributes));
      }
      $output .= "</li>\n";
    }
    $output .= '</ul>';
